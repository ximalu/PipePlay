...
**日期**：2026-06-25
**目的**：修改缓冲区间设置的行为，从"报错拒绝"改为"自动联动修正"。当修改一个值导致差距不足30秒时，自动调整另一个值。

**行为变化**：

| 场景 | 旧行为 | 新行为 |
|------|--------|--------|
| min=20→30, max=50 | 弹错误，拒绝 | max自动→60 |
| max=60→50, min=30 | 弹错误，拒绝 | min自动→20 |
| min=20→30, max=80 | 通过（差距50≥30） | 通过，不动max |

**涉及文件**：

| 文件 | 改动 |
|------|------|
| `VideoAudioSettingsFragment.java` | `validateBufferMin`/`validateBufferMax` 从拒绝改为联动修正 |
